...
.. Find `python.exe` in the installation directory (VS installs to: C:\Program Files (x86)\Microsoft Visual Studio\Shared\Python36_64\python.exe)
.. Open command line to the directory, containing `python.exe`
.. Execute `.\python.exe -m pip install "tensorflow==1.10.\*"` or `.\python.exe -m pip install "tensorflow-gpu==1.10.*"` if you want GPU acceleration via CUDA (NVidia only)
. Check the installation by launching `python.exe`, and running [source,python]`import tensorflow`. It should succeed.
...
with `new Session(...).UseSelf(sess => sess.do_stuff())`

TensorFlow API, being built on Python, use special *enter* and *exit* methods for the same purpose
.NET has `IDisposable`. Problem is that in general they do not map directly to each other.
...
A full example how to use `.UseSelf` can be found in https://github.com/losttech/Gradient-Samples/blob/master/BasicMath/BasicMathProgram.cs[samples]
...
__This is an https://github.com/pythonnet/pythonnet/issues/795[issue with the underlying interop library].__
