...
If you want to use Gradient with non-default configuration (e.g. different versions instead of Python 3.6 + TensorFlow 1.10),
use one of `GradientSetup.UseEnvironment*` methods before accessing any TensorFlow methods to select the desired TensorFlow installation.

NOTE: On Linux and MacOS Gradient may fail to find correct Python interpreter. This is due to a bug in
the older version of https://www.nuget.org/packages/WhichPython[WhichPython package]. If you get
`Gradient.GradientInitializationException: Python 3.6 not found."``, install the latest version of WhichPython.
...
You must manually install the latest version of https://www.nuget.org/packages/WhichPython/[WhichPython]
package to ensure Gradient can automatically find your Python interpreter.
